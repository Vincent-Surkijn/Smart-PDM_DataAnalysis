...
% extractFeaturesSlowstream(data,csvpath) extracts features from a slowstream file
...
%    e.g. data = readtable('22_1_17 - 22_1_23.csv', 'TextType','string');
...
while(i<l+1)
    ...
    if(data{i,"ActivePower"}>30)
        ...
        while(not(found))
            ...
            if(all(data{i+1:10:min(max(size(data)),i+200),"ActivePower"}<30))
                found = true;
            end
