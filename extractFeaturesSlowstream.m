...
% Plot: findpeaks(data{:,9},'MinPeakHeight',0.3,'MinPeakDistance',500)
% [~,LOCS] = findpeaks(data{:,9},'MinPeakHeight',0.3,'MinPeakDistance',500);
...
% on/off cycle)
% If the power is greater than 0 then the file starts in the middle of an on cycle
if(data{i,"ActivePower"}>0)
...
while(i<l+1)
    ...
    % If the power is greater than 0 then it is an on cycle
    if(data{i,"ActivePower"}>0)
        ...
        % Jump to the end of the cycle
        i = i + find(data{i:l,"ActivePower"}==0, 1, 'first');
        % Do not record the cycle if it does not end before the end of the data
